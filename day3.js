...
const dirs = {
  R: (n, x, y) => range(n).map(i => [ x+i+1, y ]),
  L: (n, x, y) => range(n).map(i => [ x-i-1, y ]),
  U: (n, x, y) => range(n).map(i => [ x, y+i+1 ]),
  D: (n, x, y) => range(n).map(i => [ x, y-i-1 ])
};
...
const toPoints = path => path.reduce((coords, subPath) => {
  const lastPoint = coords.slice(-1) || [];
  const [ lastX = 0, lastY = 0 ] = lastPoint[0] || [];
  const dir = subPath[0];
  const n = parseInt(subPath.slice(1));
  const point = dirs[dir](n, lastX, lastY);

  return [...coords, ...point ];
}, []);

const intersections = (path1, path2) =>
  path1.filter(([ x1, y1 ]) =>
    path2.find(([ x2, y2 ]) => x1 === x2 && y1 === y2));

const stepsTo = (path, [ x, y ]) => path.findIndex(([ pX, pY ]) => x === pX && y === pY) + 1;

const points1 = toPoints(path1);
const points2 = toPoints(path2);
const is = intersections(points1, points2);
console.log(is);

const steps = is.map(point => stepsTo(points1, point) + stepsTo(points2, point));
console.log(steps, Math.min.apply(null, steps));
